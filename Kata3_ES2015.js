//Tercera Kata: Poker con objetos version EcmaScript 2015
...
//Clase PokerGame - juego y reglas
class PokerGame {
	constructor () {
	}
	...
	//Verifica si es Color (todas del mismo palo)
	isColor (mano) {
		let unColor=true;
		let i=0;
		while (unColor && i < 4) {
	...
	//Verifica si es escalera (todas con numeración consecutiva, incluido As-1-2-3-4 y T-J-Q-K-As)
	isStraight (mano) {
...
class Partida {
	constructor (numJugadores) {
		...
		this.juego = new PokerGame();
	}

	compara2Manos (mano1, mano2) {
		let resul1 = this.juego.ponderate(mano1);
		let resul2 = this.juego.ponderate(mano2);
		let ganador = 0;
	...
	comparaEmpate (mano1, mano2, jugada) {
		let ganador = 0;
